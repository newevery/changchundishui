...
public class ZlibInStream extends InStream {
	...
	protected int overrun(int itemSize, int nItems) throws IOException {
		if (itemSize > bufSize)
			throw new IOException("ZlibInStream overrun: max itemSize exceeded");
		if (underlying == null)
			throw new IOException("ZlibInStream overrun: no underlying stream");
	...
	private void decompress() throws IOException {
		try {
			...
			throw new IOException("ZlibInStream: inflate failed");
		}
	}
